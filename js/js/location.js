...
// Example: you can expand this with the full list
const locationData = {
...
function populateStateAndLGA(stateElId, lgaElId) {
  const stateEl = document.getElementById(stateElId);
  const lgaEl = document.getElementById(lgaElId);
  if (!stateEl || !lgaEl) return;

  stateEl.innerHTML = "<option value=''>--Choose State--</option>";
  lgaEl.innerHTML = "<option value=''>--Choose LGA--</option>";
  lgaEl.disabled = true;
  ...
  stateEl.addEventListener("change", () => {
    lgaEl.innerHTML = "<option value=''>--Choose LGA--</option>";
    lgaEl.disabled = !stateEl.value;
    if (stateEl.value && locationData[stateEl.value]) {
      locationData[stateEl.value].forEach(lga => {
